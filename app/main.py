...
import app.models as models, app.schemas as schemas, app.util as util, app.database as database, app.crud as crud, app.oauth2 as oauth2
...
app = FastAPI()
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["8"], allow_headers=["*"])
...
@app.get("/users/anime/", response_model=schemas.Anime)
...
@app.get("/users/animes/", response_model=list[schemas.Anime])
...
@app.post("/users/animes/", response_model=schemas.Anime)
